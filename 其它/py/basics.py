...
w = xmind.load("c:\\Users\\btr\\Desktop\\basics.xmind")  
w = loader.get_workbook()
s2=w.createSheet()
s2.setTitle("basics")
r2=s2.getRootTopic()
r2.setTitle("运程是怎么跑起来的")
...
'运行环境=操作系统 + 硬件':[
    ...
    {'Windows操作系统':[
        '前身是监控程序:加载和运行程序',
         ...
         {'特征':[
            '克服了除CPU以外的硬件差异',
            {'应用通过操作系统API间接控制硬件':[
                '键盘输入、显示器输出等是通过向Windows发送指令来间接实现向硬件发送指令',
    ...
    {'源代码的运行方案':[
        ...
        {'JAVA':[
            ...
            'Java编译器:将程序员编写的源代码（sample.java）转换成字节代码（sample.class）',
...
'编绎':[
    {'基础概念':[
        ...
        {'库文件':[
            ...
            {'种类':[
                '导入库:库文件存储着两个信息，一是A函数在某DLL文件中，另一个是存储着DLL文件的文件夹信息',
        ...
        {'再配置信息':[
            '在程序运行时，虚拟的内存地址会转换成实际的内存地址。链接器会在EXE文件的开头，追加转换内存地址所需的必要信息'
        ]}
    ]},
    {'链接后的EXE文件的构造':[
        '再配置信息',
...
'外围设备控制':[
    {'识别外围设备的3种方式':[
        {'I/O端口号':[
            {'基础概念':[
                'I/O 控制器:计算机主机中，附带了用来连接显示器及键盘等外围设备的连接器。而各连接器的内部，都连接有用来交换计算机主机同外围设备之间电流特性的IC',
                'I/O是Input/Output的缩写。显示器、键盘等外围设备都有各自专用的I/O控制器。I/O控制器中有用于临时保存输入输出数据的内存。这个内存就是端口',
...
xmind.save(w,"c:\\Users\\btr\\Desktop\\basics.xmind") 